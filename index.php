<?php
// index.php — Watsons Styled (Bootstrap 5 + Vue3) + Login/Register/Forgot + Token Auth + Cart
session_start();
?>
<!DOCTYPE html>
<html lang="zh-Hant">
  ...
  <div id="app">
    ...
    <div class="watsons-header d-flex justify-content-between align-items-center">
      ...
      <div>
        ...
        <button v-if="user" class="btn btn-outline-light me-2">Hi, {{ user.username }}</button>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <!-- LOGIN -->
            <div v-if="mode==='login'">
              <input class="form-control mb-3" v-model="loginForm.username" placeholder="帳號">
    ...
    createApp({
      data() {
        return {
          ...
          user: null,
          // token removed (using PHP session instead)
          token: null,
          cartOpen: false,
          products: [],
          cart: [],
      ...
      computed: {
        modalTitle() {
          return this.mode === 'login' ? '會員登入' : this.mode === 'register' ? '註冊新帳號' : '忘記密碼'
        },
        cartCount() {
      ...
      methods: {
        openModal(mode) {
          let f = this.cart.find(x => x.id === p.id)
          if (f) f.qty++
          else this.cart.push({
            ...p,
            qty: 1
