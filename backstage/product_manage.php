    ...
    if ($product_name === '' || $price < 0 || $stock < 0 || $category_id <= 0) {
        ...
        if ($action === 'create') {
            ...
            if ($stmt->execute([
                ...
                // 若有圖片 → 存成 ../src/products/categoryId/productId.jpg
                if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {

                    $dir = "../src/products/" . $category_id . "/";
...
if (isset($_GET['delete_id'])) {
    ...
    if ($delete_id > 0) {
        // 先拿分類 ID（因為圖片路徑要用到）
        $stmt = $pdo->prepare("SELECT category_id FROM product WHERE product_id = :id");
        ...
        if ($ok) {
            $img = "../src/products/" . $catId . "/" . $delete_id . ".jpg";
            if (file_exists($img)) {
                unlink($img);
            }
    ...
    <div class="container">
        ...
        <main class="content">
            ...
            <div class="form-container">
                ...
                <form method="post" enctype="multipart/form-data" action="product_manage.php<?php echo $editProduct ? '?edit_id=' . intval($editProduct['product_id']) : ''; ?>">
                    <div class="form-row">
                    ...
                    <?php if ($editProduct): ?>
                        <div class="form-row">
                            <label>目前圖片：</label>
                            <?php
                            $imgPath = "../src/products/" . $editProduct['category_id'] . "/" . $editProduct['product_id'] . ".jpg";
